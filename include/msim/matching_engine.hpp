...
public:
  ...
  // Original — returns a new MatchResult each call.
  MatchResult process(Order incoming);

  // Performance variant — reuses the trades vector inside `out`,
  // eliminating one heap allocation per order that generates trades.
  // Call proc_result_buf_.trades.clear() before each run() loop;
  // the vector retains capacity and never reallocates after warm-up.
