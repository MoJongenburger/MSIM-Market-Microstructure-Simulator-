...
private:
  static std::optional<Price> compute_mid_(std::optional<Price> bb,
                                            std::optional<Price> ba) noexcept {
  ...
  static LiveBookDepth::DepthLevel to_level_(const X& x) {
    if constexpr (requires { x.price; x.total_qty; }) {
      return {static_cast<Price>(x.price), static_cast<Qty>(x.total_qty)};
    } else if constexpr (requires { x.price; x.qty; }) {
      return {static_cast<Price>(x.price), static_cast<Qty>(x.qty)};
    } else if constexpr (requires { x.px; x.qty; }) {
      return {static_cast<Price>(x.px), static_cast<Qty>(x.qty)};
    } else if constexpr (requires { x.price; x.volume; }) {
      return {static_cast<Price>(x.price), static_cast<Qty>(x.volume)};
    } else if constexpr (requires { x.first; x.second; }) {
      return {static_cast<Price>(x.first), static_cast<Qty>(x.second)};
    } else if constexpr (requires { x.price(); x.qty(); }) {
      return {static_cast<Price>(x.price()), static_cast<Qty>(x.qty())};
    } else {
      static_assert(sizeof(X) == 0,
                    "Unsupported LevelSummary layout: add a to_level_ branch");
    }
  }
  ...
  static auto depth_levels_(const Book& book, Side side, std::size_t levels) {
    if constexpr (requires { book.depth(side, levels); }) {
      ...
      static_assert(sizeof(Book) == 0,
                    "OrderBook depth API not found");
    }
  }

  template <class Book>
  static std::vector<LiveBookDepth::DepthLevel> extract_depth_(
      const Book& book, Side side, std::size_t levels) {
  ...
  // Changed: const std::vector<Trade>& → std::span<const Trade>
  // Accepts std::vector<Trade>, SmallVector<Trade,4>, or any contiguous range.
  void update_cache_with_engine_locked_(Ts ts, std::span<const Trade> new_trades);
