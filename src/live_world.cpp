...
LiveSnapshot LiveWorld::snapshot(std::size_t max_trades) const {
  ...
  for (std::size_t i = 0; i < n; ++i)
    s.recent_trades.push_back(cache_.trades[cache_.trades.size() - 1 - i]);
  return s;
}
...
OrderAck LiveWorld::submit_order(Order o) {
  ...
  // res.trades is SmallVector<Trade,4> — converts implicitly to span
...
// Changed signature: const std::vector<Trade>& → std::span<const Trade>
// Accepts std::vector<Trade> (from flush()), SmallVector<Trade,4> (from
// process()), or an empty {} brace-init (for cache-refresh calls).
void LiveWorld::update_cache_with_engine_locked_(Ts ts,
                                                  std::span<const Trade> new_trades) {
  std::lock_guard<std::mutex> lk(cache_mtx_);
...
void LiveWorld::worker_() {
  ...
  for (Ts ts = 0; ts <= t_end; ts += dt) {
    ...
    // flush() returns std::vector<Trade> — converts implicitly to span
    ...
    for (auto& ap : agents_) {
      ...
      for (auto act : actions) {
        if (act.type == ActionType::Submit) {
          ...
          // res.trades is SmallVector<Trade,4> — converts implicitly to span
          if (!res.trades.empty())
            update_cache_with_engine_locked_(ts, res.trades);
